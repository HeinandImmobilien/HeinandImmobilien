...
def build_home():
    prefix = ""
    home_faq = [
        ("Was kostet eine Hausverwaltung bei HEINAND?", "Das hängt von Objektgröße, Einheitenzahl und Leistungsumfang ab. Im kostenlosen Erstgespräch lernen wir uns erst kennen und besprechen Ihre Situation. Passt es für beide Seiten, folgt eine Objektanalyse und ein transparentes Festangebot: ohne versteckte Zusatzkosten."),
        ...
        ("Was ist der Unterschied zwischen WEG-Verwaltung und Mietverwaltung?", "Die WEG-Verwaltung betreut das gemeinschaftliche Eigentum einer Eigentümergemeinschaft, also Dach, Fassade, Treppenhaus und Heizungsanlage. Die Mietverwaltung kümmert sich dagegen um ein einzelnes vermietetes Objekt: Mieterkorrespondenz, Nebenkostenabrechnung, Zahlungseingänge. Wer eine vermietete Eigentumswohnung besitzt, braucht meist beides, dafür gibt es die Sondereigentumsverwaltung als Schnittstelle."),
        ("Wie lange dauert ein Verwalterwechsel bei einer WEG?", "Seit der WEG-Reform reicht für die Neubestellung eines Verwalters die einfache Mehrheit der abgegebenen Stimmen in der Eigentümerversammlung, ein wichtiger Grund ist nicht mehr nötig. Vom Erstgespräch bis zur laufenden Betreuung planen wir meist sechs bis zehn Wochen ein: Angebot, Beschlussfassung, Übergabe von der Vorverwaltung."),
        ("Was ist in der Mietverwaltung konkret enthalten?", "Dazu gehören die komplette Mieterkorrespondenz, die Abwicklung von Zahlungseingängen, eine fristgerechte Nebenkostenabrechnung, die Koordination von Handwerkern bei Reparaturen sowie regelmäßiges Reporting, damit Sie jederzeit wissen, was mit Ihrem Objekt passiert."),
        ("Bis wann muss die Nebenkostenabrechnung beim Mieter sein?", "Die Abrechnung muss dem Mieter spätestens zwölf Monate nach Ende des Abrechnungszeitraums zugehen, sonst können Nachforderungen in der Regel nicht mehr geltend gemacht werden. Als Mietverwaltung achten wir konsequent auf diese Frist und auf einen rechtssicheren Umlageschlüssel."),
        ("Führen Sie auch die Eigentümerversammlung durch?", "Ja, wir übernehmen die ordnungsgemäße Einberufung, Durchführung und Protokollierung der Eigentümerversammlung, auf Wunsch auch als hybride Versammlung mit Online-Teilnahme. Beschlüsse werden rechtssicher dokumentiert und in der Beschlusssammlung geführt."),
        ("Wie läuft ein Immobilienverkauf mit HEINAND ab?", "Wir begleiten Sie von der realistischen Werteinschätzung über die Erstellung des Exposés und die Käuferauswahl bis zum Notartermin. Weil wir viele der Objekte in der Region bereits verwalten, kennen wir die tatsächlichen Marktpreise, nicht nur Online-Schätzungen."),
        ("Ist die Immobilienbewertung wirklich kostenlos?", "Ja. Sagen Sie uns die Eckdaten Ihrer Immobilie, wir melden uns innerhalb eines Werktags mit einer ersten, fundierten Einschätzung auf Basis regionaler Marktdaten und Objektzustand, ganz ohne Verpflichtung."),
        ("Muss ich als Eigentümer vor Ort in Mainz sein?", "Nein. Gerade für Kapitalanleger, die nicht in der Region wohnen, ist unsere digitale Vorgangsverwaltung gedacht: Anliegen, Belege und Reporting laufen online, Vor-Ort-Termine übernehmen wir für Sie."),
        ("Wie kann ich ein kostenloses Erstgespräch vereinbaren?", "Am schnellsten über unser Kontaktformular oder direkt über unseren Buchungskalender, dort sehen Sie sofort freie Termine, telefonisch oder per Video. Alternativ erreichen Sie uns telefonisch oder per E-Mail. Einen persönlichen Termin vor Ort sprechen wir bei Bedarf individuell ab."),
        ("Was passiert mit meinen Daten, wenn ich ein Formular ausfülle?", "Ihre Angaben werden ausschließlich zur Bearbeitung Ihrer Anfrage verwendet und uns per E-Mail zugestellt. Eine gesonderte Datenbank betreiben wir nicht. Details dazu finden Sie in unserer Datenschutzerklärung."),
    ]
    body = f'''
...
<section class="section-white">
  <div class="container">
    ...
    <div class="steps-grid steps-grid-5">
      {step("01", "Kennenlernen", "Telefonisch oder per Video sprechen wir über Ihre Situation, Ihre Ziele und Ihr Anliegen: unverbindlich und ohne Verkaufsdruck.")}
      {step("02", "Objektbegehung & Angebot", "Passt es für beide Seiten, schauen wir uns Ihr Objekt an und Sie erhalten ein transparentes, klar kalkuliertes Angebot: ohne versteckte Kosten.")}
...
def select_field(label, options, required=True):
  ...
  <select{req_attr}><option value="" disabled selected>Bitte wählen…</option>{opts}</select>
</div>'''
...
def text_field(label, placeholder="", required=True, full=False):
    ...
    if "e-mail" in label_lower or "email" in label_lower:
        input_type = "email"
    elif "telefon" in label_lower:
        input_type = "tel"
    else:
        input_type = "text"
    return f'''<div class="form-field{cls}">
  <label>{label}{star}</label>
  <input type="{input_type}" placeholder="{placeholder}"{req_attr}>
</div>'''
  ...
  <div class="container two-col">
    ...
    <div id="anfrage" class="form-card">
      ...
      <form data-contact-form data-form-name="{name}: Angebotsanfrage">
        ...
        <div class="form-grid">
          {text_field("Vorname", "", True)}
          {text_field("Nachname", "", True)}
        </div>
        <div class="form-grid">
          {text_field("Telefonnummer", "", True)}
          {text_field("E-Mail", "", True)}
        </div>
        <div class="form-grid">
          {text_field("Anmerkung (optional)", "", False, full=True)}
        </div>
...
def build_verwalterwechsel():
    ...
    steps_html = "".join([
        wechsel_step("1", [B_EIGENTUEMER, B_HEINAND],
            ...
            "Sie oder Ihr Verwaltungsbeirat nehmen Kontakt zu uns auf: telefonisch oder per Formular. Im unverbindlichen Erstgespräch lernen wir uns kennen und klären Ihre aktuelle Situation, die Größe der Gemeinschaft und den Grund für den geplanten Wechsel.",
            "ca. 30-45 Min."),
...
def build_standort(loc):
  ...
  <span class="card-link">Mehr erfahren <span aria-hidden="true">&#8594;</span></span>
    ...
    standort_faq = [
        ...
        (f"Was kostet eine Hausverwaltung in {name}?", "Das hängt von Objektgröße und Leistungsumfang ab. Im kostenlosen Erstgespräch lernen wir uns erst kennen und besprechen Ihre Situation, danach machen wir ein transparentes Festangebot: ohne versteckte Zusatzkosten."),
        (f"Übernehmen Sie auch die WEG-Verwaltung in {name}?", f"Ja, wir führen Eigentümerversammlungen für Gemeinschaften in {name} durch, ordnungsgemäß einberufen, protokolliert und mit rechtssicherer Beschlussfassung, auf Wunsch auch hybrid mit Online-Teilnahme."),
        (f"Betreuen Sie auch einzelne vermietete Eigentumswohnungen in {name}?", f"Ja, über die Sondereigentumsverwaltung übernehmen wir für Eigentümer in {name} den kompletten Mieterkontakt sowie die Schnittstelle zur jeweiligen WEG-Verwaltung, ideal für Kapitalanleger, die nicht selbst vor Ort sind."),
        (f"Wie schnell kann ich in {name} einen Verwalter wechseln?", f"Für Eigentümergemeinschaften in {name} genügt seit der WEG-Reform ein Mehrheitsbeschluss der Eigentümerversammlung. Die eigentliche Übergabe, Unterlagen, Konten und laufende Vorgänge, organisieren wir. Von der ersten Anfrage bis zur laufenden Betreuung dauert das in der Regel sechs bis zehn Wochen."),
        (f"Kann ich in {name} auch nur eine kostenlose Immobilienbewertung anfragen?", f"Ja, auch ohne laufenden Verwaltungsvertrag. Nennen Sie uns die Eckdaten Ihrer Immobilie in {name}, Sie erhalten eine fundierte Einschätzung auf Basis echter regionaler Marktdaten."),
    ]
    body = f'''
<section class="hero hero-page">
  <div class="container hero-inner">
...
{cta_banner(prefix, f"Eigentum in {name} sollte kein zweiter Job sein.", f"Im kostenlosen Erstgespräch lernen wir uns kennen und besprechen Ihre Situation in {name}, danach zeigen wir Ihnen, wie eine strukturierte Verwaltung aussieht.")}
'''
...
<section class="section-white">
  <div class="container founder-grid">
    ...
    <div class="founder-text">
      ...
      <p>Ich bin Philipp Hartung, gebürtiger Mainzer und seit sieben Jahren in der Immobilienverwaltung. Ich halte die Erlaubnis nach § 34c als Verwalter und als Makler und bilde mich laufend weiter, um fachlich auf dem aktuellen Stand zu bleiben.</p>
...
<section class="section-white">
  <div class="container" style="text-align:center;">
    <h2 style="max-width:600px;margin:0 auto 24px;">Lernen Sie uns kennen: im kostenlosen Erstgespräch, telefonisch oder per Video.</h2>
...
<section class="hero hero-page">
  <div class="container hero-inner">
    ...
    <p class="hero-lead">Schreiben Sie uns eine Nachricht oder buchen Sie direkt einen Termin für Ihr kostenloses Erstgespräch: telefonisch oder per Video.</p>
  </div>
</section>
  ...
  <div class="container two-col">
    <div id="termin" class="form-card contact-tabs">
      ...
      <div class="tab-panel" data-tab-panel="booking">
        <p style="color:#5b6472;font-size:14px;margin-bottom:16px;">Buchen Sie direkt einen passenden Termin für Ihr kostenloses Erstgespräch. Im Kalender wählen Sie zwischen einem Telefontermin oder einem Videocall und sehen sofort, welche Zeiten frei sind. Die Buchung wird umgehend bestätigt. Einen persönlichen Termin vor Ort sprechen wir bei Bedarf individuell ab.</p>
    ...
    <div>
      ...
      <div class="contact-info-list" style="margin-top:20px;">
        ...
        <div class="contact-info-item">{contact_icon("pin")}<div><strong>Menimaneweg 4, 55130 Mainz</strong></div></div>
      </div>
      <span class="eyebrow">So läuft das Erstgespräch</span>
      <ol class="process-list" style="margin-top:16px;list-style:none;padding:0;">
        <li><span class="step-num" style="font-size:20px;margin-right:8px;">1.</span><span><b>Kennenlernen:</b> Telefonisch oder per Video sprechen wir über Ihre Ziele, Ihr Anliegen und Ihre aktuelle Situation.</span></li>
        <li><span class="step-num" style="font-size:20px;margin-right:8px;">2.</span><span><b>Gemeinsame Einschätzung:</b> Passt es für beide Seiten, besprechen wir die nächsten Schritte, zum Beispiel eine Objektbegehung.</span></li>
        <li><span class="step-num" style="font-size:20px;margin-right:8px;">3.</span><span><b>Transparentes Angebot:</b> Sie erhalten im Anschluss einen konkreten Vorschlag: ohne Verpflichtung.</span></li>
      </ol>
    </div>
  </div>
</section>
...
<section class="section-white">
  <div class="container legal">
    ...
    <p>Gewerbeerlaubnis gemäß § 34c Gewerbeordnung (GewO) für folgende Tätigkeiten:</p>
    <ul>
      <li>Vermittlung des Abschlusses von Verträgen über Grundstücke, grundstücksgleiche Rechte, gewerbliche Räume und Wohnräume sowie Nachweis der Gelegenheit zu solchen Vertragsabschlüssen (§ 34c Abs. 1 Satz 1 Nr. 1 GewO, Immobilienmakler)</li>
      <li>Verwaltung des gemeinschaftlichen Eigentums von Wohnungseigentümern sowie von Mietverhältnissen über Wohnräume für Dritte (§ 34c Abs. 1 Satz 1 Nr. 4 GewO, Wohnimmobilienverwalter)</li>
    </ul>
    <p>Zuständige Erlaubnis- und Aufsichtsbehörde: Landeshauptstadt Mainz, Standes-, Rechts- und Ordnungsamt, Abteilung Öffentliche Sicherheit und Ordnung, Kaiserstraße 3-5, 55116 Mainz.</p>

    <h2>Berufshaftpflichtversicherung</h2>
    <p>Markel Insurance SE<br>Sophienstr. 26<br>80333 München<br>Räumlicher Geltungsbereich: Europäische Union</p>

    <h2>Umsatzsteuer-ID</h2>
    <p>USt-IdNr. gemäß § 27a Umsatzsteuergesetz: DE123456 <span style="color:#8a92a3;">(vorläufig, wird nach Erteilung durch das Finanzamt aktualisiert)</span></p>
...
<section class="section-white">
  <div class="container legal">
    ...
    <p>HEINAND Immobilien<br>Geschäftsführer: Philipp Hartung<br>Menimaneweg 4<br>55130 Mainz</p>
    <p>E-Mail: <a href="mailto:{EMAIL}">{EMAIL}</a> · Telefon: {PHONE}</p>
    <p>Verantwortlich im Sinne der Datenschutz-Grundverordnung (DSGVO) ist die oben genannte Stelle. Diese Datenschutzerklärung informiert Sie darüber, welche personenbezogenen Daten bei der Nutzung von heinand.de verarbeitet werden, zu welchem Zweck dies geschieht und welche Rechte Ihnen dabei zustehen.</p>

    <h2>Ihre Rechte auf einen Blick</h2>
    <p>Ihnen stehen nach Art. 15 bis 21 DSGVO folgende Rechte zu, die Sie jederzeit formlos gegenüber der oben genannten verantwortlichen Stelle geltend machen können:</p>
    <ul>
      <li><strong>Auskunft</strong> (Art. 15 DSGVO) über die von uns zu Ihrer Person verarbeiteten Daten.</li>
      <li><strong>Berichtigung</strong> (Art. 16 DSGVO) unrichtiger oder unvollständiger Daten.</li>
      <li><strong>Löschung</strong> (Art. 17 DSGVO), soweit keine gesetzlichen Aufbewahrungspflichten entgegenstehen.</li>
      <li><strong>Einschränkung der Verarbeitung</strong> (Art. 18 DSGVO).</li>
      <li><strong>Datenübertragbarkeit</strong> (Art. 20 DSGVO) für Daten, die Sie uns bereitgestellt haben.</li>
      <li><strong>Widerspruch</strong> (Art. 21 DSGVO) gegen eine auf berechtigtem Interesse (Art. 6 Abs. 1 lit. f DSGVO) beruhende Verarbeitung.</li>
      <li><strong>Widerruf einer Einwilligung</strong> mit Wirkung für die Zukunft, sofern eine Verarbeitung auf einer Einwilligung beruht.</li>
      <li><strong>Beschwerde bei einer Aufsichtsbehörde</strong>: zuständig ist insbesondere die Landesbeauftragte für den Datenschutz und die Informationsfreiheit Rheinland-Pfalz (<a href="https://www.datenschutz.rlp.de/" target="_blank" rel="noopener">www.datenschutz.rlp.de</a>) oder die Behörde an Ihrem Wohnort.</li>
    </ul>

    <h2>Hosting und Server-Logfiles</h2>
    <p>Diese Website wird bei Vercel Inc., 340 S Lemon Ave #4133, Walnut, CA 91789, USA, gehostet. Beim Aufruf der Website verarbeitet Vercel automatisch technische Zugriffsdaten (u. a. IP-Adresse, Datum und Uhrzeit des Zugriffs, aufgerufene Seite, Browsertyp, Referrer-URL), um die Website auszuliefern, den Betrieb abzusichern und Missbrauch (z. B. Überlastungsangriffe) zu erkennen. Eine Zusammenführung mit anderen Datenquellen findet nicht statt.</p>
    <p>Rechtsgrundlage ist unser berechtigtes Interesse an einem sicheren und funktionsfähigen Betrieb der Website (Art. 6 Abs. 1 lit. f DSGVO). Da Vercel Inc. mit Sitz in den USA verarbeitet, erfolgt die Übermittlung auf Grundlage des EU-US Data Privacy Framework (DPF) bzw. hilfsweise auf Grundlage von EU-Standardvertragsklauseln.</p>

    <h2>Kontaktaufnahme über unsere Formulare</h2>
    <p>Auf mehreren Seiten (u. a. bei den Leistungen, den Standortseiten und im Kontaktformular) können Sie uns über ein Formular erreichen. Dabei verarbeiten wir die von Ihnen eingegebenen Angaben: in der Regel Vorname, Nachname, Telefonnummer, E-Mail-Adresse sowie objektbezogene Angaben (z. B. Objektart, Adresse, Anliegen) und Ihre Nachricht. Die mit „*" gekennzeichneten Felder sind Pflichtangaben; ohne sie können wir Ihre Anfrage nicht bearbeiten.</p>
    <p>Ihre Angaben werden über eine Server-Funktion technisch verarbeitet und uns per E-Mail über den Dienstleister Resend, Inc., 2261 Market Street #5039, San Francisco, CA 94114, USA, zugestellt. Ihre E-Mail-Adresse wird dabei als Antwortadresse (Reply-To) hinterlegt, damit wir Ihnen direkt antworten können. Eine dauerhafte Speicherung in einer separaten Datenbank erfolgt durch uns nicht: Ihre Anfrage liegt als E-Mail in unserem Postfach vor und unterliegt dessen üblicher Aufbewahrung.</p>
    <p>Rechtsgrundlage ist die Erfüllung vorvertraglicher Maßnahmen auf Ihre Anfrage hin (Art. 6 Abs. 1 lit. b DSGVO) sowie unser berechtigtes Interesse an einer effizienten Bearbeitung Ihrer Anfrage (Art. 6 Abs. 1 lit. f DSGVO). Da Resend, Inc. mit Sitz in den USA verarbeitet, erfolgt die Übermittlung auf Grundlage des EU-US Data Privacy Framework (DPF) bzw. hilfsweise auf Grundlage von EU-Standardvertragsklauseln.</p>

    <h2>Schadensmeldung mit Fotoupload</h2>
    <p>Im Formular zur Schadensmeldung erheben wir zusätzlich zu den oben genannten Kontaktangaben die Objektadresse, die Wohneinheit, die Art des gemeldeten Schadens sowie eine Beschreibung. Optional können Sie bis zu fünf Fotos hochladen; diese werden bereits in Ihrem Browser verkleinert und ausschließlich als Anhang der Benachrichtigungs-E-Mail übermittelt: eine gesonderte Foto-Datenbank betreiben wir nicht. Verarbeitung und Übermittlung erfolgen wie im Abschnitt „Kontaktaufnahme über unsere Formulare" beschrieben, ebenso über Resend, Inc.</p>
    <p>Rechtsgrundlage ist die Erfüllung vorvertraglicher bzw. vertraglicher Maßnahmen im Rahmen der Hausverwaltung (Art. 6 Abs. 1 lit. b DSGVO) sowie unser berechtigtes Interesse an einer schnellen, korrekt priorisierten Schadensbearbeitung (Art. 6 Abs. 1 lit. f DSGVO).</p>

    <h2>Terminbuchung über Cal.com</h2>
    <p>Auf unserer Kontaktseite bieten wir die Möglichkeit, direkt online einen Termin zu buchen. Hierfür binden wir den Buchungsdienst des Anbieters Cal.com, Inc., 4 Union Square, 4th Floor, New York, NY 10003, USA, ein. Bei Nutzung dieser Funktion werden Ihre Eingaben (u. a. Name, E-Mail-Adresse, gewählter Termin) sowie technische Daten Ihres Endgeräts an Cal.com übertragen und dort verarbeitet, um die Terminbuchung zu ermöglichen und zu bestätigen.</p>
    <p>Rechtsgrundlage ist die Erfüllung vorvertraglicher Maßnahmen auf Ihre Anfrage hin (Art. 6 Abs. 1 lit. b DSGVO). Weitere Informationen finden Sie in der Datenschutzerklärung von Cal.com: <a href="https://cal.com/privacy" target="_blank" rel="noopener">cal.com/privacy</a>. Die Übermittlung in die USA erfolgt auf Grundlage von EU-Standardvertragsklauseln bzw. des EU-US Data Privacy Framework, sofern Cal.com hierfür zertifiziert ist.</p>

    <h2>Kartendarstellung und Ortssuche</h2>
    <p>Auf mehreren Seiten (u. a. Startseite, Region, Standortseiten) zeigen wir eine interaktive Karte unseres Einzugsgebiets. Diese basiert auf der Bibliothek Leaflet, die über das Content Delivery Network von Cloudflare, Inc. (cdnjs.cloudflare.com) geladen wird, sowie auf Kartenkacheln des Anbieters CARTO (basemaps.cartocdn.com). Beim Laden der Karte wird Ihre IP-Adresse technisch bedingt an diese Anbieter übertragen, damit die Kartenkacheln bzw. Skriptdateien an Ihren Browser ausgeliefert werden können.</p>
    <p>Die Ortssuche („Sind wir auch in Ihrem Ort tätig?") funktioniert für Orte außerhalb unserer festen Ortsliste anders: Ihre Eingabe wird nicht direkt an einen Drittanbieter gesendet, sondern zunächst an unsere eigene Server-Funktion übermittelt. Diese fragt die Entfernung serverseitig bei der Geokodierungs-API von OpenStreetMap (Nominatim, betrieben von der OpenStreetMap Foundation) ab. Ihre IP-Adresse wird dabei nicht an OpenStreetMap übertragen: sichtbar ist dort nur die Server-Adresse unseres Hosting-Anbieters.</p>
    <p>Rechtsgrundlage für die Kartendarstellung und die Ortssuche ist unser berechtigtes Interesse an einer verständlichen, nutzerfreundlichen Darstellung unseres Einzugsgebiets (Art. 6 Abs. 1 lit. f DSGVO).</p>

    <h2>Google Fonts</h2>
    <p>Zur einheitlichen Darstellung von Schriftarten binden wir Google Fonts ein, einen Dienst der Google Ireland Limited, Gordon House, Barrow Street, Dublin 4, Irland. Beim Aufruf unserer Website wird dazu eine Verbindung zu Servern von Google aufgebaut, wodurch Ihre IP-Adresse an Google übertragen wird. Weitere Informationen finden Sie in der Datenschutzerklärung von Google: <a href="https://policies.google.com/privacy" target="_blank" rel="noopener">policies.google.com/privacy</a>.</p>
    <p>Rechtsgrundlage ist unser berechtigtes Interesse an einer einheitlichen und performanten Darstellung der Website (Art. 6 Abs. 1 lit. f DSGVO).</p>

    <h2>Cookies und lokale Speicherung</h2>
    <p>Wir verwenden ausschließlich technisch notwendige Cookies bzw. vergleichbare Speichertechnologien (Local Storage), etwa um Ihre Cookie-Entscheidung zu speichern. Ein Tracking zu Werbe- oder Analysezwecken durch Drittanbieter findet auf dieser Website nicht statt. Sollte sich dies zukünftig ändern, passen wir diese Datenschutzerklärung entsprechend an und holen, soweit erforderlich, vorab Ihre Einwilligung ein.</p>

    <h2>Empfänger und Auftragsverarbeiter im Überblick</h2>
    <ul>
      <li><strong>Vercel Inc.</strong> (USA): Hosting der Website und der serverseitigen Funktionen.</li>
      <li><strong>Resend, Inc.</strong> (USA): Versand der Formular-Benachrichtigungen per E-Mail.</li>
      <li><strong>Cal.com, Inc.</strong> (USA): Online-Terminbuchung, sofern genutzt.</li>
      <li><strong>Google Ireland Limited</strong> (Irland): Bereitstellung von Google Fonts.</li>
      <li><strong>Cloudflare, Inc.</strong> (USA): Auslieferung der Kartenbibliothek über CDN.</li>
      <li><strong>CARTO</strong>: Bereitstellung der Kartenkacheln.</li>
      <li><strong>OpenStreetMap Foundation</strong>: Geokodierung für die Ortssuche (serverseitig angebunden, siehe oben).</li>
    </ul>
    <p>Mit als Auftragsverarbeiter tätigen Dienstleistern schließen wir, soweit erforderlich, Verträge zur Auftragsverarbeitung nach Art. 28 DSGVO ab. Eine Weitergabe Ihrer Daten zu Werbezwecken oder ein Verkauf Ihrer Daten an Dritte findet nicht statt.</p>

    <h2>Internationale Datenübermittlungen</h2>
    <p>Ein Teil der oben genannten Dienstleister verarbeitet Daten in den USA. Für Übermittlungen in die USA stützen wir uns, soweit die jeweiligen Anbieter entsprechend zertifiziert sind, auf das EU-US Data Privacy Framework (Angemessenheitsbeschluss der EU-Kommission vom 10.07.2023). Ist ein Anbieter nicht zertifiziert, erfolgt die Übermittlung auf Grundlage von EU-Standardvertragsklauseln. Weitere Informationen zum Data Privacy Framework finden Sie unter <a href="https://www.dataprivacyframework.gov/" target="_blank" rel="noopener">dataprivacyframework.gov</a>.</p>

    <h2>Speicherdauer</h2>
    <p>Wir speichern personenbezogene Daten nur so lange, wie es für die jeweiligen Zwecke erforderlich ist oder gesetzliche Aufbewahrungsfristen (z. B. aus dem Handels- oder Steuerrecht) dies verlangen. Anfragen über unsere Formulare verbleiben als E-Mail in unserem Postfach, bis sie für die Bearbeitung nicht mehr benötigt werden. Server-Logfiles unseres Hosting-Anbieters werden im dort üblichen, technisch bedingten Umfang vorgehalten.</p>

    <h2>Automatisierte Entscheidungsfindung</h2>
    <p>Eine automatisierte Entscheidungsfindung einschließlich Profiling im Sinne von Art. 22 DSGVO findet nicht statt. Die automatische Einordnung der Dringlichkeit im Schadensmeldungsformular ist eine einfache, transparente Zuordnung anhand der von Ihnen gewählten Schadensart und dient ausschließlich Ihrer Information: sie führt zu keiner Sie betreffenden rechtlichen oder ähnlich erheblichen Entscheidung.</p>

    <h2>Änderungen dieser Datenschutzerklärung</h2>
    <p>Wir passen diese Datenschutzerklärung an, sobald Änderungen an der Website oder den eingesetzten Diensten dies erforderlich machen. Es gilt jeweils die zum Zeitpunkt Ihres Besuchs aktuelle Fassung.</p>
    <p style="color:#8a92a3;font-size:13px;margin-top:24px;">Stand: 21. Juli 2026</p>
  </div>
</section>
'''
...
def _removed_old_datenschutz_placeholder():
    unused = '''
    ...
    <ul><li>Leistungsempfänger und Auftraggeber.</li><li>Beschäftigte.</li><li>Kommunikationspartner.</li><li>Nutzer.</li><li>Dritte Personen.</li><li>Hinweisgeber.</li></ul>
    <h3>Zwecke der Verarbeitung</h3>
    <ul><li>Kommunikation.</li><li>Sicherheitsmaßnahmen.</li><li>Direktmarketing.</li><li>Organisations- und Verwaltungsverfahren.</li><li>Feedback.</li><li>Marketing.</li><li>Bereitstellung unseres Onlineangebotes und Nutzerfreundlichkeit.</li><li>Informationstechnische Infrastruktur.</li><li>Hinweisgeberschutz.</li><li>Öffentlichkeitsarbeit.</li><li>Absatzförderung.</li></ul>
    ...
    <ul class="m-elements"><li><strong>Bereitstellung Onlineangebot auf gemietetem Speicherplatz: </strong>Für die Bereitstellung unseres Onlineangebotes nutzen wir Speicherplatz, Rechenkapazität und Software, die wir von einem entsprechenden Serveranbieter (auch "Webhoster" genannt) mieten oder anderweitig beziehen; <strong>Rechtsgrundlagen:</strong> Berechtigte Interessen (Art. 6 Abs. 1 S. 1 lit. f) DSGVO).</li><li><strong>Erhebung von Zugriffsdaten und Logfiles: </strong>Der Zugriff auf unser Onlineangebot wird in Form von sogenannten "Server-Logfiles" protokolliert. Zu den Serverlogfiles können die Adresse und der Name der abgerufenen Webseiten und Dateien, Datum und Uhrzeit des Abrufs, übertragene Datenmengen, Meldung über erfolgreichen Abruf, Browsertyp nebst Version, das Betriebssystem des Nutzers, Referrer URL (die zuvor besuchte Seite) und im Regelfall IP-Adressen und der anfragende Provider gehören. Die Serverlogfiles können zum einen zu Sicherheitszwecken eingesetzt werden, z. B. um eine Überlastung der Server zu vermeiden (insbesondere im Fall von missbräuchlichen Angriffen, sogenannten DDoS-Attacken), und zum anderen, um die Auslastung der Server und ihre Stabilität sicherzustellen; <strong>Rechtsgrundlagen:</strong> Berechtigte Interessen (Art. 6 Abs. 1 S. 1 lit. f) DSGVO). <strong>Löschung von Daten:</strong> Logfile-Informationen werden für die Dauer von maximal 30 Tagen gespeichert und danach gelöscht oder anonymisiert. Daten, deren weitere Aufbewahrung zu Beweiszwecken erforderlich ist, sind bis zur endgültigen Klärung des jeweiligen Vorfalls von der Löschung ausgenommen.</li></ul>
    ...
    <ul class="m-elements"><li><strong>Verarbeitete Datenarten:</strong> Meta-, Kommunikations- und Verfahrensdaten (z. B. IP-Adressen, Zeitangaben, Identifikationsnummern, beteiligte Personen).</li><li><strong>Betroffene Personen:</strong> Nutzer (z. B. Webseitenbesucher, Nutzer von Onlinediensten).</li><li><strong>Rechtsgrundlagen:</strong> Berechtigte Interessen (Art. 6 Abs. 1 S. 1 lit. f) DSGVO). Einwilligung (Art. 6 Abs. 1 S. 1 lit. a) DSGVO).</li></ul>
    <p><strong>Weitere Hinweise zu Verarbeitungsprozessen, Verfahren und Diensten:</strong></p>
    <ul class="m-elements"><li><strong>Verarbeitung von Cookie-Daten auf Grundlage einer Einwilligung: </strong>Wir setzen eine Einwilligungs-Management-Lösung ein, bei der die Einwilligung der Nutzer zur Verwendung von Cookies oder zu den im Rahmen der Einwilligungs-Management-Lösung genannten Verfahren und Anbietern eingeholt wird. Dieses Verfahren dient der Einholung, Protokollierung, Verwaltung und dem Widerruf von Einwilligungen, insbesondere bezogen auf den Einsatz von Cookies und vergleichbaren Technologien, die zur Speicherung, zum Auslesen und zur Verarbeitung von Informationen auf den Endgeräten der Nutzer eingesetzt werden. Im Rahmen dieses Verfahrens werden die Einwilligungen der Nutzer für die Nutzung von Cookies und die damit verbundenen Verarbeitungen von Informationen, einschließlich der im Einwilligungs-Management-Verfahren genannten spezifischen Verarbeitungen und Anbieter, eingeholt. Die Nutzer haben zudem die Möglichkeit, ihre Einwilligungen zu verwalten und zu widerrufen. Die Einwilligungserklärungen werden gespeichert, um eine erneute Abfrage zu vermeiden und den Nachweis der Einwilligung gemäß der gesetzlichen Anforderungen führen zu können. Die Speicherung erfolgt serverseitig und/oder in einem Cookie (sogenanntes Opt-In-Cookie) oder mittels vergleichbarer Technologien, um die Einwilligung einem spezifischen Nutzer oder dessen Gerät zuordnen zu können. Sofern keine spezifischen Angaben zu den Anbietern von Einwilligungs-Management-Diensten vorliegen, gelten folgende allgemeine Hinweise: Die Dauer der Speicherung der Einwilligung beträgt bis zu zwei Jahre. Dabei wird ein pseudonymer Nutzer-Identifikator erstellt, der zusammen mit dem Zeitpunkt der Einwilligung, den Angaben zum Umfang der Einwilligung (z. B. betreffende Kategorien von Cookies und/oder Diensteanbieter) sowie Informationen über den Browser, das System und das verwendete Endgerät gespeichert wird; <strong>Rechtsgrundlagen:</strong> Einwilligung (Art. 6 Abs. 1 S. 1 lit. a) DSGVO).</li></ul>
    ...
    <p>Die Empfänger haben das Recht, erteilte Einwilligungen jederzeit zu widerrufen oder der werblichen Kommunikation jederzeit kostenlos über die oben genannten Kontaktmöglichkeit zu widersprechen.</p>
    ...
    <ul class="m-elements"><li><strong>Verarbeitete Datenarten:</strong> Nutzungsdaten (z. B. Seitenaufrufe und Verweildauer, Klickpfade, Nutzungsintensität und -frequenz, verwendete Gerätetypen und Betriebssysteme, Interaktionen mit Inhalten und Funktionen). Meta-, Kommunikations- und Verfahrensdaten (z. B. IP-Adressen, Zeitangaben, Identifikationsnummern, beteiligte Personen).</li><li><strong>Betroffene Personen:</strong> Nutzer (z. B. Webseitenbesucher, Nutzer von Onlinediensten).</li><li><strong>Zwecke der Verarbeitung und berechtigte Interessen:</strong> Bereitstellung unseres Onlineangebotes und Nutzerfreundlichkeit.</li><li><strong>Aufbewahrung und Löschung:</strong> Löschung entsprechend Angaben im Abschnitt "Allgemeine Informationen zur Datenspeicherung und Löschung". Speicherung von Cookies von bis zu 2 Jahren (Sofern nicht anders angegeben, können Cookies und ähnliche Speichermethoden für einen Zeitraum von zwei Jahren auf den Geräten der Nutzer gespeichert werden.).</li><li><strong>Rechtsgrundlagen:</strong> Einwilligung (Art. 6 Abs. 1 S. 1 lit. a) DSGVO). Berechtigte Interessen (Art. 6 Abs. 1 S. 1 lit. f) DSGVO).</li></ul>
